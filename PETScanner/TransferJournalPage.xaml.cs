...
public partial class TransferJournalPage : ContentPage
{
    string _journalId;
    

    public TransferJournalPage(string journalId)
    {
        InitializeComponent();
    ...
    private void OnGridSelectionChanged(object sender, Syncfusion.Maui.DataGrid.DataGridSelectionChangedEventArgs e)
{
    if (e.AddedRows.Count > 0 && e.AddedRows[0] is OnhandData selectedItem)
    {
            ...
            Navigation.PushAsync(new LocationUpdatePage(selectedItem, journalId));
    }
}
    ...
    private void OnCellTapped(object sender, Syncfusion.Maui.DataGrid.DataGridCellTappedEventArgs e)
    {
        ...
        private async void OnJournalPostClicked(object sender, EventArgs e)
    {
        var httpClient = new HttpClient();
        _journalId = _journalId.Replace("\"", "");

        ApiEndpoints endpoints = await ApiService.LoadEndpointsAsync();
        var validateTransferJournalUrl = endpoints.ValidateTransferJournalUrl;

        string baseUri = SettingsService.DefaultUrl + SettingsService.DefaultPort + validateTransferJournalUrl;
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                await DisplayAlert("succès", "Journal transferé et validé", "OK");
                await Navigation.PushAsync(new MainPage());
            }
            else
            {
                //error here in validation
    ...
    private async void OnCancelTransfertJournalPostClicked(object sender, EventArgs e)
    {
